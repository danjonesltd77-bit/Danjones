<x-layouts::auth :title="__('Confirm password')">
    <div class="text-2xl font-medium">{{ __('Confirm Password') }}</div>
    ...
    @if (session('status') || $errors->any())
        <div role="alert" class="alert relative border rounded-[0.6rem] my-7 flex items-center border-primary/20 bg-primary/5 px-4 py-3 leading-[1.7] text-primary">
            <div class="mr-2">
                <i data-lucide="shield-check" class="h-6 w-6 fill-primary/10 stroke-[0.8]"></i>
            </div>
            <div class="ml-1 mr-8 text-sm">
                @if (session('status'))
                    {{ session('status') }}
                @else
                    {{ __('Please confirm your password to proceed.') }}
                @endif
            </div>
        </div>
    @endif
        ...
        <div>
            ...
            <div x-data="{ show: false }" class="relative">
                ...
                >
                    ...
                    <svg x-show="show" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: none;"><path d="M9.88 a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.52 13.52 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" y1="2" x2="22" y2="22"/></svg>
                </button>
            </div>
        </div>
